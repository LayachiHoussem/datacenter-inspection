<?php
view('layouts.header', ['title' => 'New Inspection Sheet']);
view('layouts.sidebar');
view('layouts.navbar', ['title' => 'Datacenter Devices Inspection Sheet']);
?>
...
<form action="/inspections/store" method="POST" enctype="multipart/form-data">
    <?= csrf_field() ?>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <a href="/inspections" class="btn btn-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i> Cancel</a>
        </div>

        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 16px;">
            <div class="form-group" style="margin-bottom:0;">
                <label class="form-label" for="title">Inspection Title</label>
                <input type="text" id="title" name="title" class="form-control" value="Datacenter Hardware Equipment Sweep - <?= date('Y-m-d H:i') ?>" required>
            </div>
            ...
            <div class="form-group" style="margin-bottom:0;">
                ...
                <select id="status" name="status" class="form-control">
                    ...
                    ?>
                        <option value="<?= sanitize($stVal) ?>" <?= $stVal === 'completed' ? 'selected' : '' ?>>
                            <?= sanitize($stLabel) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <span style="font-size: 0.85rem; color: var(--text-secondary);">Dynamic inventory loaded from <code>equipment</code> table</span>
        </div>
                            ...
                            <tbody>
                                <?php foreach ($group['devices'] as $device): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="status-check-group">
                                                ...
                                                foreach ($checklistResults as $resKey => $meta): 
                                                    ...
                                                    $isDefaultChecked = ($resKey === 'na') || ($firstRes && !isset($checklistResults['na']));
                                                    $firstRes = false;
                                                ?>
                                                    <input type="radio" id="<?= $resKey ?>_<?= $device['id'] ?>" name="items[<?= $device['id'] ?>][status]" value="<?= sanitize($resKey) ?>" <?= $isDefaultChecked ? 'checked' : '' ?>>
                                        ...
                                        <td>
                                            <input type="text" name="items[<?= $device['id'] ?>][notes]" class="form-control" placeholder="Add remarque notes..." style="padding: 6px 10px; font-size: 0.84rem;">
                                        </td>

                                        <td>
                                            <div class="photo-upload-wrapper">
                                                <label for="photo_<?= $device['id'] ?>" class="btn-upload-photo" title="Upload device photo evidence">
                                                ...
                                                <img id="preview_<?= $device['id'] ?>" class="photo-preview-mini" style="display:none;" alt="Preview">
    ...
    <div class="card">
        ...
        <div class="form-group">
            ...
            <textarea id="overall_notes" name="overall_notes" class="form-control" rows="2" placeholder="Enter general datacenter facility remarks..."></textarea>
        </div>

        <button type="submit" class="btn btn-success" style="width: 100%; padding: 12px; font-size: 0.95rem; font-weight: 600;">
            <i class="fa-solid fa-circle-check"></i> Submit Inspection & Generate Report
        </button>
    </div>
</form>
